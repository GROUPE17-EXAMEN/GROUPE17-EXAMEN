        ...
        private void ListMatiereForm_Load(object sender, EventArgs e)
        {
            ...
            using (var db = new DBExamenContext())
            {
                var matieres = db.matieres
                                 .Where(m => m.cours.Any(c => c.Id == coursId)) // Vérifier si la matière est associée au cours
                                 .Select(m => new
                                 {
                                     m.Id,
                                     m.NomMatiere
                ...
                if (matieres.Any())
                {
                    dataGridViewListMatiere.DataSource = matieres; // Assurez-vous que dataGridViewListMatieres est le bon DataGridView
                }
                else
                {
